...
_DEFAULT_PDF = _BASE_DIR / "浦发上海浦东发展银行西安分行个金客户经理考核办法.pdf"
...
# 读取 PDF（路径相对本脚本所在目录，避免从其它 cwd 运行时找不到文件）
pdf_reader = PdfReader(str(_DEFAULT_PDF))
# 提取文本和页码信息
text, char_page_mapping = extract_text_with_page_numbers(pdf_reader)
print('text=',text)
print('char_page_mapping=',char_page_mapping)
