...
# 支持中文：从本机已注册字体中选第一个可用的（避免 Heiti SC / SimHei 名称找不到）
def _setup_matplotlib_cjk_font() -> None:
    ...
    for fam in registered:
        if any(
            k in fam
            for k in ("PingFang", "Heiti", "Songti", "Hiragino", "Noto Sans CJK", "YaHei")
        ):
            plt.rcParams["font.sans-serif"] = [fam]
            plt.rcParams["axes.unicode_minus"] = False
            return
...
df = pd.read_csv(BASE_DIR / "Seattle_Hotels.csv", encoding="latin-1")
# 数据探索
print(df.head())
